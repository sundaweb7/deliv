  ...
  <x-admin.table>
    ...
    @foreach($orders as $o)
      <tr>
        ...
        <td class="py-2">
          <a href="{{ route('admin.orders.wa_logs', $o['id']) }}"><x-admin.button variant="muted" class="btn-sm">WA Logs</x-admin.button></a>
          @if($o['payment_method'] === 'bank_transfer' && $o['payment_status'] !== 'paid')
            <form method="POST" action="{{ route('admin.orders.markPaid', $o['id']) }}" style="display:inline">@csrf<button type="submit" class="ml-2"><x-admin.button variant="primary" class="btn-sm">Mark Paid</x-admin.button></button></form>
          @endif
        </td>
      </tr>
    @endforeach
